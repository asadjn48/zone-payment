...
export default function AdminFleetsPage() {
  ...
  const fleets = [
    { name: 'City Logistics Ltd', vehicles: 24, subStatus: 'Active' },
    { name: 'Express Delivery Co', vehicles: 15, subStatus: 'Active' },
    { name: 'Quick Transport Services', vehicles: 8, subStatus: 'Expired' },
    { name: 'Metro Freight Solutions', vehicles: 32, subStatus: 'Active' },
    { name: 'Regional Cargo Ltd', vehicles: 12, subStatus: 'Active' },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 pb-10 mt-4 max-w-7xl">
      <h1 className="text-2xl font-bold text-foreground mb-2">Manage Fleets</h1>

      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
        
        {/* Top Controls Row */}
        <div className="p-6 border-b border-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/50">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span>Show</span>
            <input type="text" className="w-12 text-center border border-border rounded py-1 outline-none focus:ring-1 focus:ring-primary bg-white" placeholder="10" />
            <span>entries</span>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              {filteredFleets.length === 0 ? (
                 <tr><td colSpan={4} className="text-center py-10 text-gray-500">No fleets found.</td></tr>
              ) : (
                filteredFleets.map((fleet, i) => (
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors text-gray-600">
                    <td className="px-6 py-4 font-bold text-foreground">{fleet.name}</td>
        ...
        <Pagination totalItems={filteredFleets.length} />
