...
export default function FleetDetailsPage() {
  const [activeTab, setActiveTab] = useState<'vehicles' | 'payments' | 'subscription'>('vehicles');
  const params = useParams();

  // Mock fleet object (in reality, you'd fetch this using params.id)
  const fleet = {
    initials: 'CL',
    name: 'City Logistics Ltd',
    email: 'admin@citylogistics.com',
    ...
    registered: 'Jan 10, 2025',
    totalVehicles: 24,
    pendingPayments: '£340.25', // Updated to Pending Payments
    subStatus: 'Active'
  };
  ...
  return (
    <div className="flex flex-col gap-6 pb-10 mt-4 max-w-7xl">
      ...
      {/* Back Link */}
      <Link href="/admin/fleets" className="text-primary text-sm font-medium hover:underline inline-flex items-center gap-1 w-fit">
        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
        Back to Fleets
      </Link>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm p-6 md:p-8 flex flex-col sm:flex-row justify-between items-start gap-6">
        ...
        <div className="flex flex-col items-end gap-3 w-full sm:w-auto">
          ...
          <button className="w-full sm:w-auto px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 text-sm font-medium rounded-full transition-colors border border-red-100 shadow-sm">
            Deactivate Fleet
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-bold text-foreground">{fleet.totalVehicles}</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-bold text-orange-500">{fleet.pendingPayments}</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-bold text-foreground">{fleet.subStatus}</h3>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden mt-2">
        ...
        <div className="flex border-b border-border overflow-x-auto hide-scrollbar px-2 bg-gray-50/30">
          <button onClick={() => setActiveTab('vehicles')} className={`px-6 py-4 text-sm font-bold flex items-center gap-2 whitespace-nowrap border-b-2 transition-colors ${activeTab === 'vehicles' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>
            Manage Vehicles
          </button>
        ...
        <div className="p-0">
          ...
          {activeTab === 'vehicles' && (
             <div className="flex flex-col">
               {/* Vehicles Control Bar */}
               <div className="p-4 border-b border-border flex justify-between items-center bg-white">
                 <h3 className="font-bold text-foreground">Fleet Vehicles ({fleet.totalVehicles})</h3>
                 <button className="px-4 py-2 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-lg shadow-sm transition-colors flex items-center gap-2">
               ...
               <table className="w-full text-left text-sm whitespace-nowrap">
                 ...
                 <tbody className="divide-y divide-border">
                   {[
                     { reg: 'TR12 ABC', make: 'Ford Transit', driver: 'John Smith', status: 'Active' },
                     { reg: 'MJ57 YXB', make: 'Renault Master', driver: 'David Brown', status: 'Active' },
                     { reg: 'PL08 WHK', make: 'VW Transporter', driver: 'Unassigned', status: 'Inactive' },
                   ].map((v, i) => (
                     <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                       ...
                       <td className="px-6 py-4">
                         {v.driver === 'Unassigned' ? (
                           <span className="text-orange-500 font-medium italic flex items-center gap-1.5">
                             <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                             Unassigned
                           </span>
                         ) : (
                           <Link href={`/admin/users/${v.driver.toLowerCase().replace(/\s+/g, '-')}`} className="text-primary hover:underline font-medium">{v.driver}</Link>
                         )}
                       </td>
                       <td className="px-6 py-4"><Badge variant={v.status === 'Active' ? 'success' : 'warning'}>{v.status}</Badge></td>
                       <td className="px-6 py-4 text-right space-x-3">
                         <button className="text-xs font-semibold text-primary hover:text-blue-700 transition-colors">Edit Details</button>
                         <button className="text-xs font-semibold text-primary hover:text-blue-700 transition-colors">Assign Driver</button>
          ...
          {activeTab === 'subscription' && (
            <div className="p-6 md:p-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-12 mb-8">
                <div className="bg-gray-50 p-4 rounded-xl border border-border">
                  <p className="text-xs text-gray-500 mb-1">Plan</p>
                  <p className="font-bold text-foreground text-sm">Enterprise Fleet</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-border">
                  <p className="text-xs text-gray-500 mb-1">Status</p>
                  <p className="font-bold text-green-600 text-sm">Active</p>
              ...
              {/* Warning Box */}
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800 flex items-center gap-3 shadow-sm">
                <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" className="shrink-0"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                <span className="font-medium">An expired subscription will automatically deactivate all associated fleet vehicles until renewed.</span>
              </div>
            </div>
          )}
