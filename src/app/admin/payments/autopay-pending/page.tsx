...
// 1. Move static mock data outside to prevent re-creation on every render
const initialQueue = [
  { id: 'AP-1023', reg: 'ABC 1234', provider: 'Manchester Airport', charge: 15.00, balance: 25.00, status: 'Sufficient' },
  { id: 'AP-1024', reg: 'GHI 2345', provider: 'London ULEZ', charge: 12.50, balance: 5.00, status: 'Insufficient' },
  { id: 'AP-1025', reg: 'LMN 8901', provider: 'Birmingham CAZ', charge: 3.00, balance: 10.00, status: 'Sufficient' },
...
// 2. Reusable Sub-Component for Table Rows (Keeps main component extremely light)
const AutoPayTableRow = ({ item, onProcess, onNotify }: any) => (
  <tr className="hover:bg-gray-50/50 transition-colors text-gray-600">
    ...
    <td className="px-6 py-4 text-right">
      {item.status === 'Sufficient' ? (
        <button onClick={() => onProcess(item.reg, item.provider)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded shadow-sm transition-colors">Process</button>
      ) : (
...
export default function AutoPayPendingPage() {
  ...
  // Bring in our clean, reusable Toast logic
  ...
  // Action Handlers
  const handleProcess = (reg: string, provider: string) => showToast(`Payment processed for ${reg} (${provider}). Moved to Successful Payments.`, 'success');
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              {filteredQueue.length === 0 ? (
                ...
                filteredQueue.map((item) => (
                  <AutoPayTableRow key={item.id} item={item} onProcess={handleProcess} onNotify={handleNotify} />
                ))
              )}
            </tbody>
          </table>
