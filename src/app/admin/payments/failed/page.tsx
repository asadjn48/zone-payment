...
export default function FailedPaymentsPage() {
  ...
  // Updated Mock Data: Status strictly reflects the two failure reasons
  const initialFailed = [
    { id: '105668', date: '23 April 2024', reg: 'ABC 2226', driver: 'Adam White', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Manual', amount: 1200.00, status: 'Expired - Driver failed to pay within window' },
    { id: '106578', date: '23 April 2024', reg: 'PQ12 OPS', driver: 'Cesar Gomez', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Basic', amount: 125.00, status: 'Expired - Driver failed to pay within window' },
    { id: '108311', date: '23 April 2024', reg: 'EFG 1035', driver: 'Felicio Niller', provider: 'Birmingham CAZ', zone: 'Airport', tier: 'Manual', amount: 125.00, status: 'Expired - Driver failed to pay within window' },
    { id: '106993', date: '14 April 2024', reg: 'XYZ 4585', driver: 'Sarah Cooper', provider: 'Birmingham CAZ', zone: 'Airport', tier: 'Auto-Pay', amount: 1200.00, status: 'Expired - Insufficient wallet balance' },
    { id: '108110', date: '21 April 2024', reg: 'MW-6901', driver: 'Emma Rogers', provider: 'Coventry CAZ', zone: 'CAZ', tier: 'Auto-Pay', amount: 120.00, status: 'Expired - Insufficient wallet balance' },
  ];
  ...
  const manualCount = filteredFailed.filter(p => p.tier === 'Manual' || p.tier === 'Basic').length;
  const autoCount = filteredFailed.filter(p => p.tier === 'Auto-Pay').length;
    ...
    <div className="flex flex-col gap-6 pb-10 mt-4 max-w-7xl">
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-border flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-red-50/30">
          ...
          {/* Dynamic Metrics Card using Reusable Component */}
          <div className="min-w-[320px]">
             <MetricCard 
                variant="red"
                value={`£${totalAmount.toLocaleString('en-GB', { minimumFractionDigits: 2 })}`}
                label={`Failed Payments (${filteredFailed.length} transactions)`}
                ...
                subtitle={
                  <div className="flex justify-between text-xs text-gray-500 mt-3 pt-3 border-t border-border">
                    <span className="font-medium">Manual Expired: {manualCount}</span>
                    <span className="font-medium text-purple-600">Auto Expired: {autoCount}</span>
                  </div>
                }
              />
          </div>
              ...
              {filteredFailed.length === 0 ? (
                ...
                filteredFailed.map((p) => (
                  <tr key={p.id} className="hover:bg-red-50/40 transition-colors text-gray-600 bg-red-50/10">
                    ...
                    <td className="px-6 py-4 text-right font-bold text-foreground">£{p.amount.toFixed(2)}</td>
                    <td className="px-6 py-4">
                      <span className="text-red-600 font-medium text-xs truncate max-w-50 block">{p.status}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      {/* Navigational action only (no operations) */}
                      <Link href={`/admin/payments/failed/${p.id}`} className="text-primary hover:underline font-medium text-xs">View Details</Link>
        ...
        <Pagination totalItems={filteredFailed.length} />
