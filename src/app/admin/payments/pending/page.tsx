...
//   // Updated Mock Data: Strict status logic based on Tier
//   const initialPending = [
//     { id: '108496', date: '23 Apr 2024', reg: 'ABC 2224', driver: 'Sarah Miller', provider: 'Manchester Airport', zone: 'Airport', tier: 'Manual', amount: 85.50, status: 'Driver Notified' },
//     { id: '108678', date: '23 Apr 2024', reg: 'EFG 1035', driver: 'Adam', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Manual', amount: 12.50, status: 'Driver Notified' },
//     { id: '108311', date: '23 Apr 2024', reg: 'CVS 2044', driver: 'Emma Rogers', provider: 'Birmingham CAZ', zone: 'CAZ', tier: 'Auto-Pay', amount: 120.00, status: 'Payment In Progress' },
//     { id: '108221', date: '22 Apr 2024', reg: 'GIM 2346', driver: 'Nathan Cooper', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Basic', amount: 12.50, status: 'Driver Notified' },
//   ];
...
//               label={`Pending Payments (${filteredPending.length} transactions)`}
...
//                     <td className="px-6 py-4 text-right font-bold text-orange-600">£{p.amount.toFixed(2)}</td>
//                     <td className="px-6 py-4">
//                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${p.status === 'Payment In Progress' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
//                          {p.status}
//                        </span>
//                     </td>
//                     <td className="px-6 py-4 text-right">
...
//         <Pagination totalItems={filteredPending.length} />
...
export default function PendingPaymentsPage() {
  ...
  // Removed "Status" column data entirely as requested (it's now a read-only list)
  const initialPending = [
    { id: '108496', date: '23 Apr 2024', reg: 'ABC 2224', driver: 'Sarah Miller', provider: 'Manchester Airport', zone: 'Airport', tier: 'Manual', amount: 85.50 },
    { id: '108678', date: '23 Apr 2024', reg: 'EFG 1035', driver: 'Adam White', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Manual', amount: 12.50 },
    { id: '108311', date: '23 Apr 2024', reg: 'CVS 2044', driver: 'Emma Rogers', provider: 'Birmingham CAZ', zone: 'CAZ', tier: 'Auto-Pay', amount: 120.00 },
    { id: '108221', date: '22 Apr 2024', reg: 'GIM 2346', driver: 'Nathan Cooper', provider: 'London ULEZ', zone: 'ULEZ', tier: 'Basic', amount: 12.50 },
  ];
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto grow min-h-100">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              {filteredPending.length === 0 ? (
                <tr><td colSpan={9} className="text-center py-10 text-gray-500">No pending payments match your filters.</td></tr>
              ) : (
                filteredPending.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors text-gray-600">
                    <td className="px-6 py-4">{p.date}</td>
